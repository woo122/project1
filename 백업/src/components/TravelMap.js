    ...
    dailySchedule.forEach(day => {
      if (day?.activities) {
        ...
        day.activities.forEach(activity => {
          // location이 있는 모든 활동(공항, 관광지, 음식점, 숙소 등)
          if (activity.location && (activity.type === 'airport' || activity.type === 'attraction' || activity.type === 'meal' || activity.type === 'accommodation')) {
            markers.push({
              ...activity,
  ...
  const routePaths = useMemo(() => {
    ...
    dailySchedule.forEach(day => {
      if (day?.activities) {
        const dayPath = [];
        day.activities.forEach(activity => {
          // attraction, meal, airport, accommodation 타입만 경로에 포함
