...
import { Container, Paper, Alert, CircularProgress, Backdrop, Box, Typography } from '@mui/material';
...
const PlanPage = () => {
  ...
  const handleFormSubmit = async (travelInfo) => {
    ...
    try {
      ...
      let enrichedItinerary = await enrichItineraryWithTravelTime(withRestaurants);
      ...
      // 숙소 추가
      enrichedItinerary = addAccommodationToItinerary(enrichedItinerary, travelInfo.accommodation);
      console.log('🏨 Accommodation added to rule-based itinerary');
      
      console.log('✅ 일정 생성 완료, 페이지 이동 중...', enrichedItinerary);
      
      // 일정 결과 페이지로 이동
      navigate('/itinerary', { state: { itinerary: enrichedItinerary } });
    } catch (error) {
