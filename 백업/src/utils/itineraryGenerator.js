import { japanDestinations, durationBasedTrips, seasonalRecommendations, tokyoWards, tokyoWardAttractions } from '../data/japanData';
...
export const generateItinerary = async (travelInfo) => {
  ...
  const isFood = travelStyles.includes('food');
  const isShopping = travelStyles.includes('shopping');
  const isRelaxation = travelStyles.includes('relaxation');
  const isActivity = travelStyles.includes('activity');
